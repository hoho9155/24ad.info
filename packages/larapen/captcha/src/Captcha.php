...
class Captcha
{
	 ...
	 * @return bool
	 */
	public function check_api($value, $key, $config = 'default')
	: bool
	{
	 ...
	 * @return string
	 */
	public function src(string $config = 'default')
	: string
	{
		$prefix = isAdminPanel() ? config('larapen.admin.route', 'admin') . '/' : '';
		return url($prefix . 'captcha/' . $config) . '?' . $this->str->random(8);
	}
	 ...
	 * @return string
	 */
	public function img(string $config = 'default', array $attrs = [])
	: string
	{
		$attrs_str = '';
