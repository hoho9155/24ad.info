...
@if ($xPanel->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @if ($field_language !== 'en')
        <script charset="UTF-8" src="{{ asset('assets/plugins/datepicker/locales/bootstrap-datepicker.'.$field_language.'.js') }}"></script>
    @endif
    <script>
		if (jQuery.ui) {
		...
		jQuery(document).ready(function($){
			$('[data-bs-datepicker]').each(function(){
				...
				var $fake = $(this),
					$field = $fake.parents('.input-group').find('input[type="hidden"]'),
					$customConfig = $.extend({
						format: 'dd/mm/yyyy'
				...
				$picker.on('show hide change', function(e){
					if( e.date ){
						...
						try {
							...
							if( $fake.val() ){
								PNotify.defaultStack.close(); /* Close All Notices (Default Stack) */
								new PNotify.alert({
									title: 'Whoops!',
