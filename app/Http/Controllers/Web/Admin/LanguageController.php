...
class LanguageController extends PanelController
{
	public function setup()
	{
		...
		*/
		 ...
		 */
		$this->xPanel->addField([
			'label'             => trans('admin.locale_code_label'),
			'name'              => 'locale',
			'type'              => 'select2_from_array',
	...
	public function create()
	{
		if (empty(getLocales('installed'))) {
			$message = trans('admin.empty_locales_list', ['field' => trans('admin.locale_code_label')]);
	...
	private function languagesList(): array
	{
		...
		return collect($entries)->map(function ($name, $code) {
			$name = $name . ' (' . $code . ')';
	 ...
	 * @return array
	 */
	private function localesList(): array
	{
		$entries = getLocalesWithName();
		
		return collect($entries)->map(function ($name, $code) {
			return $name . ' &rarr; ' . $code;
		})->toArray();
	}
